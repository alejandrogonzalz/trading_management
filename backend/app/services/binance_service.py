...
def get_open_orders():
    try:
        ...
        if not isinstance(raw_orders, list): return []
        ...
        for tid, meta in all_meta.items():
            if meta.get('status') == 'CLOSED': continue
            tracked_ids = [o['id'] for o in meta.get('orders', [])]
            list_id = meta.get('orderListId')
            trade_legs = [o for o in relevant_orders if o['orderId'] in tracked_ids or (list_id and o.get('orderListId') == list_id)]
            ...
            if trade_legs:
            ...
            elif meta.get('status') in ['ACTIVE', 'MANUAL_CONTROL', 'PROTECTION_FAILED']:
                ...
                try:
                    ...
                    if asset_balance >= (orig_qty * 0.1) and asset_balance > 0.000001:
                        final_list.append({
                            "symbol": meta['symbol'], "orderId": f"POS_{tid}", "clientOrderId": tid,
                            "price": meta.get('entry_price', 0), "origQty": asset_balance,
                            "status": "FILLED/HOLDING" if meta.get('status') == 'ACTIVE' else meta.get('status'), 
                            "side": meta['side'], "type": "POSITION", "smart_meta": meta
                        })
                except: pass

        for o in relevant_orders:
            if o['orderId'] not in processed_order_ids: final_list.append(o)
        return final_list
...
def create_smart_trade(symbol: str, quantity: float, buy_price: Optional[float], take_profit_price: float, stop_loss_price: float, side: str = "BUY", mode: str = "SPOT"):
    ...
    try:
        ...
        # 1. Entry Order
        entry_params = {"symbol": symbol, "side": SIDE_BUY if side == "BUY" else SIDE_SELL, "type": ORDER_TYPE_MARKET if not buy_price else ORDER_TYPE_LIMIT, "quantity": formatted_qty, "recvWindow": 60000, "newClientOrderId": client_order_id}
        if buy_price:
        ...
        if entry_order.get('status') == 'FILLED':
            ...
            if fills:
                ...
                if fee_asset == asset_bought: actual_received_qty = total_qty - total_entry_fees
        ...
        if client_order_id in all_trades:
            all_trades[client_order_id].update({'entry_price': save_price, 'quantity': actual_received_qty, 'status': 'ACTIVE', 'entry_fees': total_entry_fees, 'fee_asset': fee_asset})
        ...
        try:
            if has_tp and has_sl:
                oco_params = {"symbol": symbol, "side": SIDE_SELL if side == "BUY" else SIDE_BUY, "quantity": oco_qty, "listClientOrderId": f"LIST_{client_order_id}", "aboveType": "LIMIT_MAKER", "belowType": "STOP_LOSS_LIMIT", "abovePrice": format_price(symbol, take_profit_price), "belowStopPrice": format_price(symbol, stop_loss_price), "belowPrice": format_price(symbol, stop_loss_price), "belowTimeInForce": TIME_IN_FORCE_GTC, "recvWindow": 60000}
                try:
                    ...
                    if "aboveType" in str(e): oco_res = binance_client._post("orderList/oco", True, data=oco_params)
                    else: raise e
            elif has_tp:
                exit_order = binance_client.create_order(symbol=symbol, side=SIDE_SELL if side == "BUY" else SIDE_BUY, type=ORDER_TYPE_LIMIT, timeInForce=TIME_IN_FORCE_GTC, quantity=oco_qty, price=format_price(symbol, take_profit_price), newClientOrderId=f"TP_{client_order_id}", recvWindow=60000)
...
def market_close_position(symbol: str, quantity: Optional[float] = None, order_list_id: Optional[int] = None, client_order_id: Optional[str] = None):
    try:
        sync_binance_time()
        if order_list_id:
            try: binance_client._delete("orderList", True, data={"symbol": symbol, "orderListId": order_list_id, "recvWindow": 60000})
            except: pass
        if client_order_id:
            ...
            if meta:
                for o in meta.get('orders', []):
                    if o['role'] in ['TP', 'SL']:
                        try: binance_client.cancel_order(symbol=symbol, orderId=o['id'], recvWindow=60000)
                        except: pass
        time.sleep(1.5)
        info = binance_client.get_account(recvWindow=60000)
        ...
        if sell_qty > real_balance: sell_qty = real_balance
        price_info = binance_client.get_symbol_ticker(symbol=symbol)
        if (sell_qty * float(price_info['price'])) < 5.1:
            _mark_trade_closed(symbol, order_list_id, client_order_id, quantity, float(price_info['price']), 0)
        ...
        exit_price, total_exit_fees, actual_close_time, exit_fee_asset = 0, 0, None, None
        ...
        _mark_trade_closed(symbol, order_list_id, client_order_id, float(formatted_qty_str), exit_price, total_exit_fees, actual_close_time, exit_fee_asset)
        return sell_order
    except Exception as e: raise HTTPException(status_code=400, detail=str(e))

def _mark_trade_closed(symbol: str, order_list_id: Optional[int] = None, client_order_id: Optional[str] = None, quantity: float = 0, exit_price: float = 0, exit_fees: float = 0, close_time: Optional[float] = None, exit_fee_asset: Optional[str] = None):
    all_trades = trade_tracker._load_trades()
...
def format_quantity(symbol: str, quantity: float) -> str:
    try:
        ...
        precision = len(str(step_size).split('.')[-1].rstrip('0')) if step_size < 1.0 else 0
        factor = 10 ** precision
...
def format_price(symbol: str, price: float) -> str:
    try:
        ...
        precision = len(str(tick_size).split('.')[-1].rstrip('0')) if tick_size < 1.0 else 0
        return "{:0.{}f}".format(price, precision)
    except: return "{:0.2f}".format(price)

def get_symbols():
    try:
        info = binance_client.get_exchange_info()
        return sorted([s['symbol'] for s in info['symbols'] if (s['symbol'].endswith('USDC') or s['symbol'].endswith('USDT')) and s['status'] == 'TRADING'])
    except Exception as e:
