...
async def get_deep_langgraph_analysis(symbol: str, indicators: dict[str, Any], mode: str = "SPOT") -> dict[str, Any]:
    """
    ...
    try:
        ...
        for tf, tf_data in indicators.items():
            if isinstance(tf_data, dict):
                filtered_indicators[tf] = {k: v for k, v in tf_data.items() if k in essential_keys}
                # Ensure there's at least a price field
                if "close" not in filtered_indicators[tf] and "price" not in filtered_indicators[tf]:
        ...
        print(f"LangGraph request payload size: {payload_size} bytes, timeframes: {list(filtered_indicators.keys())}")
        ...
        async with httpx.AsyncClient(timeout=60.0) as http_client:
            ...
            for attempt in range(max_retries):
                try:
                    response = await http_client.post(f"{LANGGRAPH_URL}/analyze", json=payload, timeout=60.0)
                    if response.status_code == 200:
                        try:
                            result = response.json()
                            print(f"LangGraph Raw Response (JSON) attempt {attempt + 1}: {result}")
                        except json.JSONDecodeError:
                            print(f"LangGraph returned non-JSON response: {response.text}")
                        ...
                        if not has_critical_data:
                            print(f"Empty response from LangGraph (attempt {attempt + 1}/{max_retries})")
                            if attempt < max_retries - 1:
                                await asyncio.sleep(2**attempt)
                                continue
                            return {"error": "LangGraph returned empty analysis after retries"}
                        ...
                        final_lev = int(setup.get("leverage")) if setup.get("leverage") else None
                        ...
                        print(f"LangGraph Error: {response.status_code} - {response.text}")
                        if attempt < max_retries - 1:
                            await asyncio.sleep(2**attempt)
                            continue
                        return {"error": f"LangGraph service returned status {response.status_code}"}
                except httpx.RequestError as exc:
                    print(f"LangGraph Connection Error (attempt {attempt + 1}): {type(exc).__name__}: {exc}")
                    if attempt < max_retries - 1:
                        await asyncio.sleep(2**attempt)
                        continue
                    return {"error": f"LangGraph connection failed: {exc}"}
                except Exception as e:
                    print(f"Unexpected error in LangGraph call (attempt {attempt + 1}): {e}")
                    if attempt < max_retries - 1:
                        await asyncio.sleep(2**attempt)
                        continue
