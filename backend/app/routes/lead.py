...
from app.models import CancelOrderRequest, FuturesCloseRequest, LeadOrderRequest, SetLeverageRequest
...
def get_lead_binance_history(symbol: str | None = None):
...
def create_lead_order(req: LeadOrderRequest):
    ...
    return futures_service.create_lead_order(
        symbol=req.symbol, side=req.side, order_type=req.type, quantity=req.quantity, price=req.price
    )
