    ...
    try:
        ...
        for symbol in top_pairs:
            try:
                ...
                active_tids = [
                    tid
                    for tid, m in all_meta.items()
                    if m["symbol"] == symbol and m["status"] in ["ACTIVE", "PROTECTION_FAILED"]
                ]
                ...
                close_res = binance_service.market_close_position(
                    symbol=symbol, quantity=meta["quantity"], order_list_id=meta.get("orderListId"), client_order_id=tid
                )

                if (
                    close_res.get("status") == "FILLED"
                    or close_res.get("status") == "ARCHIVED"
                    or "orderId" in close_res
                ):
                    print("✅ Panic Sell SUCCESSFUL")
                else:
                ...
                if final_meta["status"] == "CLOSED":
                    pnl = (
                        (final_meta.get("exit_price", 0) - final_meta.get("entry_price", 0))
                        / final_meta.get("entry_price", 1)
                    ) * 100
