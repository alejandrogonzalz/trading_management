...
def process_single_pair(pair: str, base_tf: str) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        indicators = indicator_service.calculate_indicators(candles)
        if indicators.get("error"): return None
...
def run_scan(pairs: List[str], base_tf: str = "1h") -> List[Dict[str, Any]]:
    ...
    print(f"Turbo-Scan completed in {(datetime.datetime.now() - datetime.datetime.fromisoformat(scan_start_time)).total_seconds():.2f} seconds.")
    return results
