...
Para evitar esta circularidad, las m\'etricas financieras se calculan mediante una simulaci\'on \textit{forward-looking} que \textbf{ignora} los niveles de salida predichos por el modelo y utiliza exclusivamente el indicador de volatilidad ATR (disponible al momento de la decisi\'on, calculado con datos pasados):
...
Los multiplicadores 1.5 y 1.0 establecen una relaci\'on riesgo-recompensa de 1.5:1, est\'andar en \textit{trading} sistem\'atico~\cite{pardo2008}. Expresar los niveles en m\'ultiplos de ATR ---y no en montos fijos--- hace que se adapten autom\'aticamente a la volatilidad de cada activo y momento: se ensanchan en mercados agitados y se estrechan en mercados tranquilos. El SL a 1 ATR sit\'ua la p\'erdida l\'imite justo fuera del rango de fluctuaci\'on normal de una vela, evitando cierres por ruido; el TP a 1.5 ATR fija un objetivo que, con esa relaci\'on riesgo-recompensa, requiere acertar solo el 40\,\% de las operaciones para no perder capital ---margen amplio frente a la exactitud direccional observada. Adem\'as, este mismo m\'ultiplo de ATR es el que emplea el etiquetador para definir la direcci\'on (\S2), lo que mantiene la coherencia entre las etiquetas y la simulaci\'on.
...
El dataset descarta el 78\,\% de las velas crudas mediante cinco filtros de calidad cuantitativos. Este dise\~no responde a dos motivaciones: primero, la alineaci\'on con el caso de uso operativo, donde el sistema de producci\'on aplica un nodo evaluador que rechaza \textit{setups} antes de ejecutar \'ordenes; segundo, la imposibilidad de etiquetar muestras ambiguas sin introducir ruido en el dataset. En consecuencia, la exactitud sobre el dataset filtrado aplica al subconjunto de \textit{setups} pre-seleccionados, no a condiciones arbitrarias de mercado. El efecto cuantitativo de este sesgo de selecci\'on se mide expl\'icitamente en la ablaci\'on del filtro de \textit{drawdown} (\S\ref{sec:ablation-drawdown}).
...
Un detalle metodol\'ogico relevante para interpretar las curvas de p\'erdida (Figura~\ref{fig:ablation-loss}) es la estrategia de \textit{loss masking} --- es decir, qu\'e tokens de cada secuencia de entrenamiento contribuyen al c\'alculo del gradiente. En este trabajo se emple\'o \textbf{p\'erdida de secuencia completa}: la entrop\'ia cruzada se calcula sobre \emph{todos} los tokens de la secuencia (prompt del sistema, prompt del usuario y respuesta del asistente), sin enmascarar la porci\'on de entrada.
...
Otras direcciones complementarias incluyen: la \textbf{incorporaci\'on de an\'alisis de sentimiento} ---res\'umenes de noticias, Twitter/X o Reddit a\~nadidos al \textit{prompt}--- aprovechando que el modelo procesa texto de forma nativa, para capturar el componente narrativo que los indicadores t\'ecnicos no reflejan; el \textbf{\textit{fine-tuning} del modelo de 14B} para una comparaci\'on de escala controlada frente al de 7B (la versi\'on de 14B es la que opera en producci\'on); una \textbf{prueba de estr\'es} sobre mercados bajistas y eventos extremos (colapso de LUNA, contagio de FTX en 2022), ausentes del per\'iodo de prueba predominantemente alcista; un \textit{ensemble} h\'ibrido que combine la direcci\'on del LLM con salidas basadas en reglas de los modelos ML; la aplicaci\'on de DPO (\textit{Direct Preference Optimization}) usando se\~nales de rendimiento como \textit{reward}; y el re-entrenamiento con los campos categ\'oricos removidos para cerrar la cuesti\'on de la asimetr\'ia de informaci\'on (\S\ref{sec:ablation-occlusion}).
