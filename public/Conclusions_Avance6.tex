...
% Forzar "Tabla" (babel-spanish rotula las tablas como "Cuadro" por defecto)
...
\twocolumn[{%
\begin{center}
{\LARGE\bfseries Evaluaci\'on experimental del ajuste fino (\textit{fine-tuning}) mediante QLoRA frente a modelos cl\'asicos para la predicci\'on direccional en criptomonedas\par}
\vspace{0.8em}
{\small Alejandro Gonz\'alez Almaz\'an \quad Dra. Alicia Fernanda Galindo Manrique\\[0.3em]
Tecnol\'ogico de Monterrey --- Maestr\'ia en Inteligencia Artificial Aplicada\par}
\vspace{0.8em}
\rule{\textwidth}{0.4pt}
\end{center}
...
La predicci\'on de la direcci\'on del precio en criptomonedas es un problema de baja relaci\'on se\~nal-ruido en el que los modelos cl\'asicos de aprendizaje autom\'atico rara vez superan el 65\,\% de exactitud. Este trabajo eval\'ua experimentalmente si el ajuste fino (\textit{fine-tuning}) de un modelo de lenguaje mediante QLoRA mejora dicha predicci\'on. Sobre datos OHLCV de 12 criptomonedas durante 18 meses, etiquetados con un algoritmo retrospectivo y divididos mediante una partici\'on temporal estricta con embargo, se compara el modelo ajustado contra el mismo modelo sin ajustar (\textit{zero-shot}) y contra modelos cl\'asicos (XGBoost, Random Forest y LSTM). Sobre el conjunto sin filtro de \textit{drawdown} ---sin sesgo de supervivencia--- el modelo ajustado alcanza \textbf{84.13\,\% de exactitud direccional} y un \textit{profit factor} de \textbf{2.09} bajo una simulaci\'on de operaciones basada en ATR sin informaci\'on futura, frente a 0.70 del mismo modelo en modo \textit{zero-shot}: el ajuste fino convierte un sistema que pierde capital en uno rentable. La mejora es estad\'isticamente significativa (McNemar, $\chi^2=1{,}949$, $p\approx0$). C\'odigo: \url{https://github.com/alejandrogonzalz/trading_management}.
\vspace{0.8em}

\begin{center}
\includegraphics[width=0.92\textwidth]{figures/fig_pipeline_etl.png}
\captionof{figure}{Proceso de construcci\'on del conjunto de datos: extracci\'on de velas OHLCV, c\'alculo de indicadores t\'ecnicos, etiquetado retrospectivo con filtros de calidad y partici\'on temporal 70/15/15 con embargo.}
\label{fig:pipeline}
\end{center}
\vspace{0.8em}
}]
...
Los \textbf{indicadores t\'ecnicos} son transformaciones matem\'aticas de los datos OHLCV que producen se\~nales cuantificables sobre la condici\'on del mercado ---tendencia, volatilidad, momentum--- a partir exclusivamente de datos pasados. La Secci\'on~\ref{sec:indicators} describe los indicadores utilizados; varios de ellos (ATR, RSI, ADX) provienen del trabajo cl\'asico de Wilder~\cite{wilder1978}.
...
Los indicadores se agrupan por funci\'on. La \textbf{tendencia} se captura con tres medias m\'oviles exponenciales (EMA de 20, 50 y 200 periodos, corto/medio/largo plazo) y el \'indice direccional medio (ADX-14, valores $>$25 indican tendencia fuerte). El \textbf{momentum} se mide con el RSI-14 (sobrecompra $>$70 / sobreventa $<$30) y el MACD (12/26/9). La \textbf{volatilidad} se cuantifica con el ATR-14 (en unidades de precio) y las bandas de Bollinger (20, 2$\sigma$). El \textbf{volumen} se normaliza contra su media m\'ovil de 20 periodos. Los indicadores de momentum y volatilidad siguen las definiciones originales de Wilder~\cite{wilder1978}.

\textbf{Caracter\'isticas derivadas.} A partir de los indicadores base se computan cuatro descriptores: la alineaci\'on de medias m\'oviles (\textit{heatmap} de tendencia), la estructura de precio respecto a m\'aximos recientes, el ratio de volatilidad relativa (ATR actual frente a su promedio) y el ratio de actividad de volumen. El \textit{heatmap} de tendencia es una se\~nal \emph{discreta}: dada la terna de medias exponenciales $\{E_{20}, E_{50}, E_{200}\}$ y el precio de cierre $P$, se asigna una categor\'ia ordinal $h\in\{-2,-1,0,1,2\}$ ---de fuertemente bajista a fuertemente alcista--- seg\'un cu\'antas de las desigualdades de alineaci\'on alcista ($P>E_{20}>E_{50}>E_{200}$) se satisfacen y sus sim\'etricas bajistas. El modelo de lenguaje recibe esta categor\'ia en su forma textual (``fuertemente alcista'', ``neutral'', etc.), mientras que los modelos de \'arbol la reciben \'unicamente como ese entero ordinal.
...
Se reportan cuatro m\'etricas. La \textbf{exactitud direccional} es el porcentaje de predicciones con direcci\'on correcta (LONG/SHORT). El \textbf{win rate} es la fracci\'on de operaciones que alcanzan el TP (ATR) antes que el SL (ATR). El \textbf{profit factor} es el cociente entre la suma de ganancias y el valor absoluto de la suma de p\'erdidas (porcentuales); PF$=$1.0 indica equilibrio, 1.5--2.5 es el rango profesional~\cite{pardo2008} y PF$>$5 es sospechoso de sobreajuste. El \textbf{max drawdown} es la mayor ca\'ida acumulada desde un m\'aximo de la \textit{equity} simulada.

% ── 3.5. Optimizaci\'on de hiperpar\'ametros ────────────────────────────────────
\subsection{Optimizaci\'on de hiperpar\'ametros}

Para garantizar una comparaci\'on equitativa entre los modelos de ML cl\'asico, el modelo de \textit{Deep Learning} (LSTM) y el LLM ajustado, se realiz\'o una b\'usqueda aleatoria (\textit{Randomized Search}) con validaci\'on temporal mediante \textit{TimeSeriesSplit} ($k=3$ divisiones), asegurando que en cada iteraci\'on el conjunto de validaci\'on sea estrictamente posterior al de entrenamiento~\cite{pardo2008}. Se eligi\'o b\'usqueda aleatoria sobre b\'usqueda exhaustiva porque, para espacios de 4--6 dimensiones, 80--150 iteraciones aleatorias alcanzan una probabilidad superior al 95\,\% de muestrear al menos una configuraci\'on dentro del 5\,\% superior del espacio~\cite{bergstra2012}. La Tabla~\ref{tab:gridsearch} detalla el n\'umero de iteraciones y par\'ametros explorados por cada modelo.
...
\caption{Exactitud direccional por modelo sobre el mismo conjunto de prueba temporal. El modelo de producci\'on (sin filtro, 84.13\,\%) supera al mejor modelo ML cl\'asico (62.7\,\%) por m\'as de 21\,pp y al modelo \textit{zero-shot} (56.5\,\%) por 27.6\,pp.}
\label{fig:accuracy}
\end{figure}

\subsection{Validaci\'on estad\'istica}

Para determinar si la diferencia de exactitud entre el modelo ajustado y el \textit{zero-shot} es estad\'isticamente significativa ---y no producto del azar muestral--- se emple\'o la prueba de McNemar~\cite{mcnemar1947}. Esta prueba es adecuada porque: (1)~ambos modelos se eval\'uan sobre \textbf{exactamente las mismas muestras} (datos pareados), (2)~la variable de respuesta es binaria (acierto/fallo por muestra), y (3)~no requiere supuesto de normalidad, a diferencia de la prueba~$t$, lo que la hace robusta para la comparaci\'on de clasificadores~\cite{dietterich1998,pardo2008}.
...
$$
donde $b$ es el n\'umero de muestras que el modelo ajustado clasifica correctamente y el \textit{zero-shot} no, y $c$ el caso inverso. La prueba se realiza sobre el modelo de producci\'on (sin filtro de \textit{drawdown}) frente al \textit{zero-shot} evaluado sobre el mismo dataset sin filtro, comparando las 11,294 muestras de prueba pareadas. Se observ\'o $b=4{,}060$ y $c=938$ (raz\'on 4.3:1 a favor del modelo ajustado), produciendo, con la correcci\'on de continuidad de Yates~\cite{edwards1948}, $\chi^2 = 1{,}948.9$ ($p\approx0$). La hip\'otesis nula ---que ambos modelos tienen la misma tasa de error--- se rechaza rotundamente.

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{figures/fig5_mcnemar.png}
\caption{Pares discordantes de la prueba de McNemar (modelo sin filtro vs.\ \textit{zero-shot}, 11,294 muestras pareadas): 4,060 aciertos exclusivos del modelo ajustado frente a 938 del \textit{zero-shot} sustentan $\chi^2=1{,}949$, $p\approx0$.}
\label{fig:mcnemar}
\end{figure}

\subsection{Modelos de \textit{fine-tuning} evaluados}

Se reportan dos modelos ajustados con hiperpar\'ametros id\'enticos (lr=$2\times10^{-5}$, rank=16, alpha=32, 2 \'epocas con \textit{early stopping}). La diferencia entre ellos es el conjunto de entrenamiento: el primero usa el dataset sin filtro de \textit{drawdown} (sin sesgo de supervivencia), y el segundo el dataset filtrado con el nombre del s\'imbolo anonimizado. Conviene precisar que ambos modelos se entrenaron con las etiquetas de salida del etiquetador retrospectivo; la circularidad metodol\'ogica no se elimina en el entrenamiento sino \emph{en la evaluaci\'on}, donde las m\'etricas financieras se recalculan con la simulaci\'on basada en ATR descrita anteriormente, que ignora los niveles de salida predichos por el modelo. La Tabla~\ref{tab:qlora_configs} resume sus resultados.

\begin{table}[H]
\centering
\caption{Modelos QLoRA de producci\'on: resultados (simulaci\'on ATR)}
\label{tab:qlora_configs}
\footnotesize
\begin{tabularx}{\columnwidth}{lXX}
\toprule
 & \textbf{Sin filtro} & \textbf{Anonimizado} \\
\midrule
\textbf{Exactitud (test)} & \textbf{84.13\%} & \textbf{88.66\%} \\
Win rate (ATR) & 66.3\% & 81.7\% \\
Profit factor (ATR) & 2.09 & 4.63 \\
Max drawdown (ATR) & 49.6\% & 27.0\% \\
Train acc.\ (diagn.) & 88.5\% & 87.0\% \\
Gap (train$-$test) & $+$4.4pp & $-$1.7pp \\
$N$ evaluadas & 11,294 & 8,425 \\
\bottomrule
\end{tabularx}
{\scriptsize Ambos modelos comparten hiperpar\'ametros; la \'unica variable es el dataset de entrenamiento.}
\end{table}

El modelo sin filtro constituye el resultado m\'as conservador y defendible: se entrena y eval\'ua sin el sesgo de supervivencia del filtro de \textit{drawdown}, por lo que su exactitud de 84.13\,\% y su \textit{profit factor} de 2.09 reflejan el desempe\~no sobre condiciones de mercado realistas. El modelo anonimizado, evaluado sobre el dataset filtrado pero sin acceso a la identidad del activo, alcanza 88.66\,\% y confirma que la ventaja no proviene de memorizar s\'imbolos espec\'ificos (ver \S\ref{sec:robustness}).
...
\subsection{Formulaci\'on del problema de entrenamiento}
\label{sec:training-formulation}

El ajuste fino se plantea como modelado de lenguaje autorregresivo sobre la secuencia \textit{chat-templated} $\mathbf{x} = (x_1, \ldots, x_T)$ (sistema + usuario + asistente). La funci\'on de p\'erdida es la entrop\'ia cruzada (log-verosimilitud negativa) de predicci\'on del siguiente token, promediada sobre las $N$ secuencias del lote de entrenamiento:
\begin{equation}
\mathcal{L}(\theta) = -\frac{1}{N}\sum_{i=1}^{N}\frac{1}{T_i}\sum_{t=1}^{T_i}
\log p_\theta\!\left(x_t^{(i)} \mid x_{<t}^{(i)}\right)
\end{equation}
computada sobre la secuencia completa, sin enmascarar la porci\'on de entrada (ver justificaci\'on en \S\ref{sec:loss-masking}).

Los par\'ametros $\theta$ que se optimizan no son los 7.6B del modelo base ---congelados en 4 bits (NF4)--- sino \'unicamente los adaptadores LoRA \cite{hu2021}. Para cada m\'odulo objetivo, el peso efectivo es
\begin{equation}
W = W_0 + \frac{\alpha}{r}BA, \quad A \in \mathbb{R}^{d_{\text{in}}\times r},\; B \in \mathbb{R}^{r \times d_{\text{out}}}
\end{equation}
con $r{=}16$, $\alpha{=}32$ y $W_0$ fija. El problema de entrenamiento es $\theta^\star = \arg\min_{\{A,B\}} \mathcal{L}(\theta)$, combinando LoRA con cuantizaci\'on de 4 bits del modelo base \cite{dettmers2023}.

La minimizaci\'on se realiza con AdamW \cite{loshchilov2019}, una variante de Adam \cite{kingma2015} con \textit{weight decay} desacoplado del gradiente:
\begin{align}
...
\end{align}
con $\beta_1{=}0.9$, $\beta_2{=}0.999$, $\epsilon{=}10^{-8}$ y $\lambda{=}0.01$ (valores por defecto). La tasa de aprendizaje $\eta$ parte de $2\times10^{-5}$ con calentamiento lineal (50 pasos) seguido de decaimiento coseno \cite{loshchilov2017}, sobre un lote efectivo de 16 secuencias (bf16).\footnote{Por completitud: los modelos cl\'asicos comparados minimizan sus propias funciones de p\'erdida ---XGBoost minimiza \textit{log-loss} v\'ia \textit{gradient boosting}; el LSTM minimiza entrop\'ia cruzada binaria v\'ia Adam \cite{kingma2015}--- por lo que la comparaci\'on de exactitud entre modelos contrasta resultados de distintos problemas de optimizaci\'on, no el mismo problema resuelto por m\'etodos distintos.}
...
Optimizador & AdamW ($\beta_1{=}0.9$, $\beta_2{=}0.999$, $\lambda{=}0.01$) \\
\'Epocas & 2 (\textit{early stopping}, paciencia=3) \\
Batch efectivo & $8 \times 2$ (lote $\times$ acumulaci\'on) = 16 \\
...
\textbf{Convergencia.} Se configuraron 2 \'epocas con \textit{early stopping} (paciencia de 3 evaluaciones sin mejora); en la pr\'actica el entrenamiento se detuvo de forma anticipada al estabilizarse la p\'erdida de validaci\'on. El proceso se ejecut\'o en una GPU de 80~GB y tom\'o aproximadamente 3.5 horas. Este r\'egimen es consistente con la pr\'actica est\'andar: Dettmers et al.~\cite{dettmers2023} reportan convergencia t\'ipica en 1--3 \'epocas para QLoRA sobre datasets de instrucci\'on, y Hu et al.~\cite{hu2021} observan que LoRA alcanza el rendimiento del \textit{fine-tuning} completo en 2--5 \'epocas.

\textbf{Evaluaci\'on completa.} Los modelos de producci\'on se eval\'uan sobre el conjunto de prueba completo (11,294 muestras para el modelo sin filtro, 8,425 para el anonimizado), sin sub-muestreo. El intervalo de confianza al 95\,\% (m\'etodo de Wilson~\cite{wilson1927}) resultante es de aproximadamente $\pm$0.7\,pp, suficientemente estrecho para sostener las conclusiones.
...
La red LSTM aprende combinaciones ponderadas de valores num\'ericos calibradas al r\'egimen estad\'istico del per\'iodo de entrenamiento. Ante un cambio de r\'egimen, esas correlaciones aprendidas dejan de ser predictivas, lo que explica su ca\'ida a 51.5\,\% (esencialmente aleatorio). Los modelos de \'arbol, en cambio, codifican umbrales discretos (por ejemplo, ``RSI~$>$~65 $\rightarrow$ SHORT'') que sobreviven desplazamientos de distribuci\'on con mayor gracia, manteniendo un 62--63\,\%.

Esta observaci\'on sustenta la hip\'otesis central: si umbrales simples generalizan mejor que secuencias aprendidas, un modelo capaz de razonar sem\'anticamente sobre los indicadores deber\'ia generalizar a\'un mejor --- y es precisamente lo observado (84\,\% vs.\ 63\,\% vs.\ 51\,\%).

\begin{figure*}[ht]
\centering
\includegraphics[width=0.82\textwidth]{figures/fig1_loss_curves.png}
\caption{Curvas de p\'erdida del \textit{fine-tuning} QLoRA: convergencia saludable, con brecha entre entrenamiento y validaci\'on inferior a 0.025.}
\label{fig:loss}
\end{figure*}
...
El contraste m\'as revelador surge al aplicar esta misma simulaci\'on ATR al modelo base en modo \textit{zero-shot} sobre el mismo dataset sin filtro: su \textit{profit factor} es de \textbf{0.70}, es decir, inferior a 1.0 --- un sistema que \emph{pierde} capital de forma sostenida. El \textit{fine-tuning} sobre exactamente los mismos datos y bajo id\'entica simulaci\'on eleva ese \textit{profit factor} a 2.09. Dado que la metodolog\'ia de salida es id\'entica en ambos casos, la diferencia se atribuye \'unicamente a la mejora en exactitud direccional que aporta la especializaci\'on de dominio: el \textit{fine-tuning} no solo incrementa el porcentaje de aciertos, sino que transforma un sistema inviable en uno rentable. Este contraste se manifiesta tambi\'en en el riesgo: bajo la misma simulaci\'on ATR, el \textit{zero-shot} acumula un \textit{drawdown} m\'aximo del 100\,\% ---la curva de \textit{equity} simulada se agota por completo, equivalente a la quiebra de la cuenta--- frente al 49.6\,\% del modelo ajustado.
...
La brecha de m\'as de 20 puntos porcentuales entre el LLM ajustado (84--88\,\%) y el mejor modelo ML (63\,\%) refleja parcialmente esta asimetr\'ia de representaci\'on. No es evidencia de un error metodol\'ogico ni de sabotaje a los modelos cl\'asicos, sino una consecuencia estructural del formato de entrada: los \'arboles de decisi\'on codifican categor\'ias como enteros ordinales con p\'erdida de informaci\'on sem\'antica, mientras que el LLM procesa esas mismas categor\'ias en su forma textual original. La secci\'on~\ref{sec:ablation-occlusion} presenta un experimento que intenta cuantificar esta contribuci\'on.

\subsection{Universo de criptomonedas evaluado}

El dataset cubre 12 criptomonedas seleccionadas por capitalizaci\'on y perfil de volatilidad (Tabla~\ref{tab:universe}): tres de gran capitalizaci\'on (BTC, ETH, BNB) con menor volatilidad relativa y mayor liquidez; cinco de capitalizaci\'on media (SOL, XRP, ADA, AVAX, DOT) con movimientos direccionales m\'as pronunciados; y cuatro de alta volatilidad (DOGE, LINK, MATIC, NEAR) susceptibles a movimientos impulsivos. Esta diversidad es relevante porque el mercado de criptomonedas exhibe propiedades estad\'isticas distintas a mercados tradicionales: colas pesadas en la distribuci\'on de retornos, correlaci\'on asim\'etrica entre activos durante eventos de liquidaci\'on y r\'egimenes que cambian abruptamente sin advertencia fundamental~\cite{bouri2017}. Un modelo que generaliza a trav\'es de estos 12 activos demuestra robustez ante distintos perfiles de volatilidad y liquidez.\footnote{MATIC aparece en el per\'iodo de entrenamiento pero no en el conjunto de prueba (febrero--mayo 2026): el activo dej\'o de cotizar bajo ese s\'imbolo tras su migraci\'on a POL a finales de 2024, por lo que el desglose por activo (Tabla~\ref{tab:persymbol}) cubre 11 de las 12 criptomonedas.}

\begin{table}[H]
\centering
\caption{Universo de 12 criptomonedas: s\'imbolo, nombre y perfil}
\label{tab:universe}
\footnotesize
\begin{tabularx}{\columnwidth}{llX}
\toprule
\textbf{S\'imbolo} & \textbf{Nombre} & \textbf{Perfil} \\
\midrule
BTC & Bitcoin & Gran capitalizaci\'on \\
ETH & Ethereum & Gran capitalizaci\'on \\
BNB & BNB & Gran capitalizaci\'on \\
\midrule
SOL & Solana & Capitalizaci\'on media \\
XRP & XRP & Capitalizaci\'on media \\
ADA & Cardano & Capitalizaci\'on media \\
AVAX & Avalanche & Capitalizaci\'on media \\
DOT & Polkadot & Capitalizaci\'on media \\
\midrule
DOGE & Dogecoin & Alta volatilidad \\
LINK & Chainlink & Alta volatilidad \\
MATIC & Polygon (migrado a POL) & Alta volatilidad \\
NEAR & NEAR Protocol & Alta volatilidad \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Interpretaci\'on conservadora del resultado}

El resultado principal de este trabajo \textbf{no es una cifra absoluta de exactitud}, sino las siguientes conclusiones relativas: (1)~el \textit{fine-tuning} mejora significativamente al \textit{zero-shot} por +27.6\,pp ($p\approx0$) sobre el \textit{mismo modelo base} --- un resultado estad\'isticamente inequ\'ivoco; (2)~los modelos ML cl\'asicos alcanzan un techo de $\sim$63\,\% independientemente de la b\'usqueda de hiperpar\'ametros, lo que sugiere un l\'imite estructural de la representaci\'on num\'erica; y (3)~el resultado de producci\'on (84.13\,\%) se mide sobre el dataset sin filtro de \textit{drawdown}, sin sesgo de supervivencia.

El nivel de exactitud depende del rigor del filtrado: sobre el dataset filtrado de alta calidad el modelo alcanza 88.66\,\%, mientras que sobre el dataset sin filtro ---condiciones de mercado m\'as ruidosas y realistas--- alcanza 84.13\,\%. Lo relevante es que \textit{bajo las mismas condiciones y el mismo dataset}, el \textit{fine-tuning} produce una mejora sustancial y reproducible. Ambos modelos se eval\'uan sobre el conjunto de prueba completo (8,425 y 11,294 muestras respectivamente), con un intervalo de confianza al 95\,\% (m\'etodo de Wilson) de aproximadamente $\pm$0.7\,pp.

\subsection{Exactitud por s\'imbolo}

La Tabla~\ref{tab:persymbol} desglosa la exactitud del modelo de producci\'on (sin filtro) por activo sobre el conjunto de prueba completo. Los 11 s\'imbolos evaluados se sit\'uan en un rango estrecho ---entre 81.8\,\% (DOT) y 86.4\,\% (SOL), una dispersi\'on de apenas 4.6\,pp--- lo que indica que el modelo no depende de un activo particular para alcanzar su desempe\~no agregado, sino que generaliza de forma homog\'enea a trav\'es de distintos perfiles de volatilidad y capitalizaci\'on.

\begin{table}[H]
\centering
\caption{Exactitud direccional del modelo de producci\'on (sin filtro) por criptomoneda}
\label{tab:persymbol}
\footnotesize
...
La Tabla~\ref{tab:literature} sit\'ua el resultado en el contexto de trabajos publicados. El rango de 84--88\,\% se ubica en el extremo alto de la literatura reportada, pero es consistente con otros resultados sobre datasets de se\~nales seleccionadas; en contraste, Pindza~\cite{pindza2026} demuestra que muchos modelos sobreajustan severamente bajo controles estrictos de fuga de informaci\'on. El resultado de este trabajo se diferencia por la evaluaci\'on temporal con embargo y por reportar el desempe\~no sobre el dataset sin filtro de \textit{drawdown} como cifra principal.
...
\midrule
Lopez-Lira y Tang~\cite{lopez2023} & GPT \textit{zero-shot}, noticias & $\sim$60\% \\
Bysik y \'Slepaczuk~\cite{bysik2026} & XGBoost/LSTM \textit{walk-forward} & $\sim$65\% \\
Pindza~\cite{pindza2026} & \textit{Gradient boosting}, WF purgado & Falla \\
\textbf{Este trabajo} & \textbf{QLoRA, sin filtro} & \textbf{84.13\%} \\
\bottomrule
\end{tabularx}
\end{table}
...
El hallazgo principal es que remover el filtro \emph{reduce} la exactitud (88.03\,$\rightarrow$\,84.13\,\%, $-$3.90\,pp), no la mejora, a pesar de entrenar sobre un dataset 34\,\% m\'as grande. Esto contradice la predicci\'on de que el filtro estar\'ia inflando artificialmente el resultado: la evidencia emp\'irica apunta a que el filtro de \textit{drawdown} es una se\~nal de calidad leg\'itima que eleva la cota alcanzable, pero no fabrica el resultado. Adem\'as, la brecha entre exactitud de entrenamiento y de prueba cambia de signo entre ambos modelos; conviene precisar, no obstante, que la brecha inversa del modelo filtrado ($-$17.5\,pp, test$>$train) proviene de un diagn\'ostico de exactitud de entrenamiento calculado sobre una muestra reducida del r\'egimen m\'as antiguo, por lo que no debe leerse como evidencia de generalizaci\'on. El modelo sin filtro, con un diagn\'ostico m\'as representativo, presenta una brecha peque\~na y positiva ($+$4.4\,pp, train$>$test), el patr\'on esperado dado el mayor ruido del dataset sin filtrar.
...
\bottomrule
\end{tabularx}
{\scriptsize Ambas configuraciones usan el dataset filtrado y los mismos hiperpar\'ametros; la \'unica variable es la presencia del nombre del s\'imbolo. La referencia ``con s\'imbolo'' se evalu\'o sobre un subconjunto estratificado y la anonimizada sobre el conjunto de prueba completo (8,425 muestras).}
\end{table}
...
El resultado (88.66\,\%) no muestra ca\'ida respecto al modelo con s\'imbolo (88.03\,\%); la diferencia de +0.63\,pp cae dentro de los intervalos de confianza de ambas estimaciones. La conclusi\'on es que la identidad del activo no contribuye a la exactitud: el modelo aprende patrones de indicadores t\'ecnicos generalizables, no asociaciones memorizadas con s\'imbolos espec\'ificos. Adicionalmente, este modelo alcanza un \textit{profit factor} de 4.63 bajo simulaci\'on ATR, el m\'as alto entre los modelos evaluados (sobre el dataset filtrado de alta calidad).

\subsubsection{Descarte de memorizaci\'on por pre-entrenamiento}
\label{sec:pretraining-memorization}

Una hip\'otesis m\'as radical es que el modelo base pudiera estar recuperando movimientos de precio \textbf{memorizados durante su pre-entrenamiento}~\cite{magar2022}, en lugar de razonar a partir de los indicadores del prompt. Este vector de fuga ser\'ia externo al pipeline de datos y partici\'on ya auditado --- los pesos del modelo ``recordando'' qu\'e ocurri\'o en una fecha y activo dados.
...
\toprule
\textbf{M\'etrica (sonda \textit{eval-only}, N=8,425)} & \textbf{Sin categ\'oricos} \\
\midrule
Exactitud (test) & 87.60\% \\
Precisi\'on / recall LONG & 0.903 / 0.843 \\
Precisi\'on / recall SHORT & 0.853 / 0.909 \\
Errores de parseo & 0 \\
\bottomrule
\end{tabularx}
{\scriptsize El modelo se entren\'o \textbf{con} los dos campos presentes y aqu\'i se eval\'ua \textbf{sin} ellos. La exactitud de referencia con los campos presentes se sit\'ua en el rango 88.0--88.7\,\% (modelos filtrados de \S\ref{sec:robustness}), de modo que la ca\'ida atribuible a los descriptores categ\'oricos es del orden de 0.4--1.1\,pp, comparable al IC 95\% ($\pm$0.69\,pp).}
\end{table}

La ca\'ida (del orden de 0.4--1.1\,pp) es comparable al intervalo de confianza de la estimaci\'on ($\pm$0.69\,pp) y las m\'etricas por clase permanecen balanceadas. Este resultado sugiere que los campos categ\'oricos \textbf{no parecen ser el motor principal} de la brecha frente a los modelos ML: el modelo mantiene pr\'acticamente la misma exactitud sin acceso a la informaci\'on textual que los \'arboles pierden al codificarla como enteros.
...
Este an\'alisis conecta con la interpretaci\'on de la brecha entre p\'erdida de entrenamiento y validaci\'on reportada en la Figura~\ref{fig:loss} (brecha train/eval inferior a 0.025). Dado que la p\'erdida incluye los tokens de prompt predecibles (loss$\approx$0), \textit{ambas} curvas (train y eval) est\'an dominadas por esos mismos tokens, lo que explica tanto su similitud como su convergencia a valores relativamente altos ($\sim$0.79): este valor refleja la entrop\'ia residual de los tokens de respuesta (que s\'i var\'ian entre muestras), no una dificultad inusual de la tarea. La ganancia estimada de adoptar enmascaramiento por completaci\'on para nuestra configuraci\'on espec\'ifica (muchas muestras, salida corta y estructurada, prompt repetitivo) es inferior a 1 punto porcentual de exactitud.

\subsection{Lectura de las curvas de p\'erdida y la brecha inversa}
\label{sec:gap-interpretation}

La Figura~\ref{fig:loss} muestra una convergencia saludable en ambos modelos de producci\'on: la p\'erdida de validaci\'on desciende mon\'otonamente hasta estabilizarse en un plateau, sin divergir de la p\'erdida de entrenamiento. El \textit{early stopping} detiene cada entrenamiento al detectar ese plateau, lo que descarta sobreajuste severo al nivel de la funci\'on de p\'erdida.

Conviene examinar la brecha entre la exactitud de entrenamiento y la de prueba, que difiere entre los dos modelos. El modelo sin filtro presenta una brecha positiva moderada ($+$4.4\,pp, train$>$test), el patr\'on esperado de una ligera especializaci\'on al conjunto de entrenamiento; el modelo anonimizado, en cambio, presenta una brecha pr\'acticamente nula ($-$1.7\,pp). En ambos casos la magnitud es peque\~na, lo que descarta memorizaci\'on: solo el 0.53\,\% de los par\'ametros se actualiza (40.4M adaptadores) y el modelo base congelado act\'ua como regularizador impl\'icito~\cite{hu2021}, limitando la capacidad efectiva pese a los 7 mil millones de par\'ametros totales.

El balance de clases del conjunto de prueba es pr\'acticamente 50/50 (LONG vs.\ SHORT) y las m\'etricas por clase son sim\'etricas, de modo que la exactitud reportada no es un artefacto de predecir la clase mayoritaria. La evaluaci\'on sobre el conjunto de prueba completo (sin sub-muestreo) refuerza la robustez estad\'istica de estas cifras.

\textbf{Sobre la magnitud de la exactitud y la brecha.} Cabe preguntarse si una exactitud tan alta acompa\~nada de una brecha tan peque\~na refleja un conjunto de datos demasiado peque\~no o una tarea trivial para un modelo sobredimensionado. Ninguna de las dos explicaciones se sostiene. El conjunto no es peque\~no para \textit{fine-tuning} ---decenas de miles de ejemplos de entrenamiento y m\'as de ocho mil de prueba---, y la tarea no es trivial: sobre exactamente los mismos datos, los modelos cl\'asicos (\'arboles, \textit{ensembles} y LSTM) no superan el $\sim$63\,\%, de modo que un problema f\'acil o con fuga residual los habr\'ia elevado tambi\'en. La coexistencia de exactitud alta y brecha peque\~na es consistente con el r\'egimen de \textit{benign overfitting} descrito para modelos sobre-parametrizados~\cite{belkin2019}, y no con la sobre-memorizaci\'on t\'ipica del \textit{fine-tuning} sin restricciones~\cite{ruan2025}: solo se actualiza el 0.53\,\% de los par\'ametros y el modelo base congelado act\'ua como regularizador, por lo que la capacidad efectiva permanece acotada. La explicaci\'on m\'as parsimoniosa es que la representaci\'on textual de los indicadores aporta una ventaja real sobre los vectores num\'ericos planos, no que la tarea sea trivial.
...
\caption{Matriz de confusi\'on del modelo de producci\'on (sin filtro) sobre las 11,294 muestras de prueba. El balance entre errores de tipo LONG$\rightarrow$SHORT (788) y SHORT$\rightarrow$LONG (1,004) confirma que el modelo no presenta sesgo hacia una clase. Precisi\'on LONG = 0.836, precisi\'on SHORT = 0.847; \textit{recall} LONG = 0.867, \textit{recall} SHORT = 0.813.}
\label{fig:confusion}
\end{figure}
...
\caption{Curvas ROC (AUC 0.68--0.69), \textit{precision-recall} y diagrama de confiabilidad de los modelos ML cl\'asicos. La calibraci\'on indica que una confianza de 8/10 corresponde a $\sim$0.8--0.9 de exactitud real. El modelo QLoRA emite una confianza poco variada (valor casi fijo en la salida JSON), por lo que estas curvas aplican \'unicamente a Random Forest y XGBoost.}
\label{fig:roc}
\end{figure*}

La Figura~\ref{fig:synthesis} resume esta progresi\'on. Las curvas de \textit{equity} simuladas (Figura~\ref{fig:equity}) muestran una acumulaci\'on de retorno sostenida para los modelos ajustados, sin los desplomes del \textit{zero-shot}; las m\'etricas financieras agregadas (Figura~\ref{fig:trade}) sit\'uan su \textit{profit factor} (2.09--4.63) en el rango profesional, lejos de la inflaci\'on de la simulaci\'on con \textit{hindsight}. La matriz de confusi\'on (Figura~\ref{fig:confusion}) confirma la ausencia de sesgo de clase y las brechas train/val/test (Figura~\ref{fig:gap}) descartan el sobreajuste. Finalmente, las curvas ROC y \textit{precision-recall} (Figura~\ref{fig:roc}) corresponden a los modelos ML cl\'asicos: su AUC de 0.68--0.69 revela una capacidad discriminativa moderada y su diagrama de confiabilidad indica una calibraci\'on razonable, un contraste \'util frente a su techo de exactitud.
...
Finalmente, la \textbf{calibraci\'on de confianza} del modelo es limitada: la salida JSON emite un valor de confianza poco variado, por lo que el modelo no proporciona una probabilidad bien calibrada que permita dimensionar posiciones de forma diferenciada. Esto se aborda como trabajo futuro mediante esquemas de \textit{position sizing} basados en se\~nales auxiliares.
...
Para habilitar el despliegue con capital real, se requiere en primer lugar de-circularizar tambi\'en el entrenamiento, reemplazando las etiquetas de salida retrospectivas (TP/SL a\'un derivadas del precio futuro) por etiquetas basadas en el ATR actual: $\text{TP} = \text{entrada} \pm 1.5 \times \text{ATR}$, $\text{SL} = \text{entrada} \mp 1.0 \times \text{ATR}$ ---la misma regla ya empleada en la evaluaci\'on. Esto alinea entrenamiento y evaluaci\'on y produce m\'etricas financieras plenamente defensibles. En segundo lugar, la simulaci\'on debe incorporar comisiones reales (0.1\,\%/lado) y \textit{slippage} estimado (5--20 puntos b\'asicos). En tercer lugar, se requiere un per\'iodo de \textit{paper trading} m\'inimo de 4 semanas con monitoreo de exactitud rodante antes de comprometer capital.
...
\bibitem{lopezprado2018}
M.~L\'opez de Prado, \textit{Advances in Financial Machine Learning}. Hoboken, NJ: Wiley, 2018.
...
\bibitem{edwards1948}
A.~L.~Edwards, ``Note on the `correction for continuity' in testing the significance of the difference between correlated proportions,'' \textit{Psychometrika}, vol.~13, no.~3, pp.~185--187, 1948.

\bibitem{wilder1978}
J.~W.~Wilder, \textit{New Concepts in Technical Trading Systems}. Greensboro, NC: Trend Research, 1978.

\bibitem{brown1992}
S.~J.~Brown, W.~Goetzmann, R.~G.~Ibbotson, and S.~A.~Ross, ``Survivorship bias in performance studies,'' \textit{Review of Financial Studies}, vol.~5, no.~4, pp.~553--580, 1992.

\bibitem{belkin2019}
M.~Belkin, D.~Hsu, S.~Ma, and S.~Mandal, ``Reconciling modern machine-learning practice and the classical bias--variance trade-off,'' \textit{Proc. National Academy of Sciences}, vol.~116, no.~32, pp.~15849--15854, 2019.

\bibitem{magar2022}
I.~Magar and R.~Schwartz, ``Data contamination: From memorization to exploitation,'' in \textit{Proc. Association for Computational Linguistics (ACL)}, 2022.
