...
Se presenta la evaluaci\'on comparativa de varios modelos de predicci\'on de direcci\'on de precio en criptomonedas, incluyendo modelos cl\'asicos de \textit{Machine Learning} (XGBoost, Random Forest, LSTM), un LLM en modo \textit{zero-shot} (Qwen~2.5~7B), y un LLM \textit{fine-tuned} mediante QLoRA sobre el mismo modelo base. El \textit{fine-tuning} se realiz\'o sobre datos OHLCV de 12 criptomonedas abarcando 18 meses, etiquetados mediante un algoritmo retrospectivo. Para evitar el sesgo de supervivencia, el resultado principal se mide sobre el conjunto sin filtro de \textit{drawdown} (75,289 muestras): sobre un \textit{holdout} temporal estricto de 81~d\'ias, el modelo ajustado alcanza \textbf{84.13\,\% de exactitud direccional} con un \textit{profit factor} de \textbf{2.09} bajo una simulaci\'on de operaciones \textit{forward-looking} basada en ATR (sin circularidad metodol\'ogica), frente a un \textit{profit factor} de 0.70 del mismo modelo en modo \textit{zero-shot} ---es decir, el \textit{fine-tuning} transforma un sistema que pierde capital en uno rentable. La superioridad sobre el \textit{zero-shot} es estad\'isticamente inequ\'ivoca ($\chi^2=557$, $p\approx0$, McNemar), y los modelos ML cl\'asicos alcanzan un techo de $\sim$64\,\% de exactitud bajo las mismas condiciones. Experimentos de robustez complementarios (anonimizaci\'on de s\'imbolo, oclusi\'on de campos) descartan explicaciones alternativas del resultado. C\'odigo: \url{https://github.com/alejandrogonzalz/trading_management}.
\vspace{0.8em}
...
Se reportan dos modelos ajustados con hiperpar\'ametros id\'enticos (lr=$2\times10^{-5}$, rank=16, alpha=32, 2 \'epocas con \textit{early stopping}), entrenados ambos con etiquetas de salida basadas en ATR para evitar la circularidad metodol\'ogica. La diferencia entre ellos es el conjunto de entrenamiento: el primero usa el dataset sin filtro de \textit{drawdown} (sin sesgo de supervivencia), y el segundo el dataset filtrado con el nombre del s\'imbolo anonimizado. La Tabla~\ref{tab:qlora_configs} resume sus par\'ametros y resultados bajo la simulaci\'on ATR.
...
El resultado principal de este trabajo \textbf{no es una cifra absoluta de exactitud}, sino las siguientes conclusiones relativas: (1)~el \textit{fine-tuning} mejora significativamente al \textit{zero-shot} por +27.6\,pp ($p\approx0$) sobre el \textit{mismo modelo base} --- un resultado estad\'isticamente inequ\'ivoco; (2)~los modelos ML cl\'asicos alcanzan un techo de $\sim$64\,\% independientemente de la b\'usqueda de hiperpar\'ametros, lo que sugiere un l\'imite estructural de la representaci\'on num\'erica; y (3)~el resultado de producci\'on (84.13\,\%) se mide sobre el dataset sin filtro de \textit{drawdown}, sin sesgo de supervivencia.
...
\caption{Matriz de confusi\'on del modelo de producci\'on (sin filtro) sobre las 11,294 muestras de prueba. El balance entre errores de tipo LONG$\rightarrow$SHORT (788) y SHORT$\rightarrow$LONG (1,004) confirma que el modelo no presenta sesgo hacia una clase. Precisi\'on LONG = 0.836, precisi\'on SHORT = 0.847; \textit{recall} LONG = 0.867, \textit{recall} SHORT = 0.847.}
\label{fig:confusion}
\end{figure}
