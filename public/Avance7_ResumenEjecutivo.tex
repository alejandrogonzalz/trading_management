...
\item \textbf{Retorno simulado positivo y consistente}: bajo la simulaci\'on hist\'orica m\'as conservadora (sin filtro de calidad), el modelo final promedia $+$0.42\,\% de retorno por operaci\'on sobre 11{,}294 operaciones en 81 d\'ias (acumulado bruto $+$4{,}768\,\%, sin reinversi\'on). Se reporta como \textbf{evidencia direccional, no como proyecci\'on de rentabilidad}: cada operaci\'on se trata como una apuesta unitaria sin \textit{compounding} ni l\'imite de capital concurrente, sin \textit{slippage} ni impacto de mercado, y el periodo de 81 d\'ias evaluado fue predominantemente alcista (\S\ref{sec:riesgos}) --- el acumulado bruto reportado no es alcanzable tal cual en operaci\'on real; debe leerse como un \textbf{ranking comparativo entre modelos} bajo id\'entica metodolog\'ia, no como un retorno proyectado.
\end{itemize}
