...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/executive_summary/fig_production_pipeline_vertical.png}
...
\item \textbf{Gobernanza de datos validada por auto-correcci\'on}: la partici\'on en entrenamiento/validaci\'on/prueba es estrictamente cronol\'ogica, con una "zona de seguridad" (\textit{embargo}) de 24 horas en cada frontera para garantizar que ninguna muestra de prueba se haya visto, ni parcialmente, durante el entrenamiento. Este rigor no fue gratuito: una auditor\'ia interna detect\'o que una iteraci\'on previa usaba una partici\'on no estrictamente temporal ---una fuga de informaci\'on futura que infla artificialmente los resultados de IA financiera--- y todas las cifras de este documento se re-midieron tras corregirla. Para el lector de negocio, el hecho relevante es doble: el resultado est\'a libre de ese sesgo, y el equipo demostr\'o que detecta y corrige sus propios errores antes de reportarlos.
\end{itemize}
...
\noindent\textit{?`C\'omo se compara 2.09 con la competencia?} Un sistema de ML cl\'asico publicado sobre criptomonedas reporta \textit{profit factor} 1.60 fuera de muestra; el mismo Qwen~2.5~7B \textbf{sin ajustar}, medido en otro estudio independiente sobre acciones, reporta apenas 1.16. El 2.09 de este trabajo no solo cae en el rango profesional de Pardo (1.5--2.5): supera a ambas referencias externas, lo que corrobora que la mejora no es un artefacto exclusivo de esta metodolog\'ia de evaluaci\'on.
...
\item \textbf{El conocimiento gen\'erico no basta; la especializaci\'on de dominio s\'i.} El mismo modelo base, sin ajuste, "sabe" parcialmente (56.5\,\%) pero no sabe operar: pierde dinero. El \textit{fine-tuning} es la diferencia entre un modelo inviable y uno rentable --- y esa diferencia es estad\'isticamente inequ\'ivoca (prueba de McNemar, $\chi^2=1{,}949$, $p\approx0$, sobre 11{,}294 decisiones pareadas). La Figura~\ref{fig:exec-confusion} hace visible esta brecha: el modelo ajustado concentra sus predicciones en la diagonal correcta, mientras que el \textit{zero-shot} las reparte casi 50/50 entre LONG y SHORT para cada clase real --- el patr\'on de un modelo que apenas discrimina direcci\'on.
\item \textbf{Elegimos el resultado conservador, no el m\'as alto.} Existe una variante del modelo que alcanza 88.66\,\% de exactitud (\textit{profit factor} 4.63), pero se entrena y eval\'ua sobre el subconjunto de \textit{setups} ya filtrados como "limpios": su cifra es optimista y su \textit{profit factor} ---por encima de 4--- entra en el territorio que la propia literatura considera sospechoso de sobreajuste. El \textbf{modelo final recomendado} (84.13\,\%, \textit{profit factor} 2.09) se entrena sobre \emph{todas} las muestras disponibles, sin ese filtro ---decisi\'on derivada del an\'alisis de robustez del Avance 6, no de la comparaci\'on inicial de familias de modelos del Avance 5---, y es por tanto el n\'umero m\'as defendible frente a condiciones de mercado reales. Adem\'as, generaliza de forma pareja: entre los 11 activos evaluados individualmente la exactitud var\'ia apenas 4.6 puntos porcentuales (81.8\,\%--86.4\,\%) --- no depende de "acertarle" a una sola criptomoneda.
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{figures/conclusion/fig6_confusion_comparison.png}
\caption{Matrices de confusi\'on del modelo ajustado (izquierda) y del \textit{zero-shot} (derecha) sobre las mismas 11{,}294 muestras de prueba. El modelo ajustado concentra la masa en la diagonal; el \textit{zero-shot} la dispersa casi por igual entre las dos predicciones para cada clase real.}
\label{fig:exec-confusion}
\end{figure}
...
Software de entrenamiento & Unsloth + TRL + PyTorch, en contenedor Docker con FlashAttention-2 preinstalada (evita compilarla, ver \S\ref{sec:cloud-vs-local}) \\
...
\item[Fase 2 --- Validaci\'on en vivo sin riesgo (4 semanas).] Antes de comprometer capital real, el propio equipo defini\'o como condici\'on obligatoria un periodo de \textit{paper trading} (operaci\'on simulada sobre datos de mercado en vivo) con monitoreo continuo de exactitud. Esto cierra la brecha entre "funciona en backtest hist\'orico" y "funciona en condiciones actuales de mercado". \textbf{Responsable}: Alejandro Gonz\'alez Almaz\'an (autor del proyecto), bajo supervisi\'on de la Dra.\ Alicia Fernanda Galindo Manrique. \textbf{Inicio}: inmediatamente despu\'es de activar la Fase~1, sin esperar a la defensa. \textbf{Criterio de avance a Fase 3 con capital real}: \textit{win rate} y \textit{profit factor} sostenidos dentro del rango profesional ya validado en \textit{backtest} (PF $\geq$ 1.5, Pardo 2008) durante las 4 semanas completas; si no se cumple, el sistema permanece en \textit{paper trading} y se reentrena antes de reintentar el \textit{gate}.
...
\end{tabularx}
\\[0.3em]
{\scriptsize $^\dagger$Supuestos: consumo de 400\,W bajo carga sostenida de entrenamiento (300\,W GPU + 100\,W resto del equipo: CPU, RAM, ventiladores) y tarifa el\'ectrica de \$0.15 USD/kWh, consistente con la tarifa residencial mexicana de CFE en rango intermedio/alto-consumo ($\sim$2.5--2.8 MXN/kWh a $\sim$18 MXN/USD). Tiempo local escalado desde el \'unico punto de referencia documentado para esta GPU ($\sim$23\,h por modelo en el Anexo de infraestructura de \texttt{Conclusions\_Avance6.tex}, equivalente a $\sim$6.4$\times$ m\'as lento que el A100 80GB). C\'alculo: $0.4\text{kW} \times 23\text{h} \times \$0.15\text{/kWh} \approx \$1.38$.}
\end{table}
...
Parte de esa brecha de velocidad no es solo la GPU: la nube eligi\'o una imagen Docker de \textbf{Unsloth} con \textbf{FlashAttention-2} ya compilada, mientras que en Windows local esa dependencia no instala de forma confiable y el entrenamiento corre en el modo de atenci\'on m\'as lento (\textit{eager}). Evitar compilar FlashAttention-2 desde fuente --- fr\'agil por incompatibilidades de versi\'on con PyTorch --- fue, junto con la GPU de mayor VRAM, lo que hizo viable completar cada corrida en horas y no en d\'ias.
...
\midrule
Hospedaje del servidor de aplicaci\'on & Instancia AWS Lightsail sin GPU (2\,GB RAM / 2 vCPU $=$ \$10/mes; 4\,GB / 2 vCPU $=$ \$20/mes), la inferencia se delega (Figura~\ref{fig:exec-lightsail}) & \$10--20 \\
Inferencia en producci\'on & $\sim$\$0.02 por cada 1{,}000 predicciones (modelo local) vs.\ $\sim$\$0.08 de la alternativa por API gestionada$^\ddagger$ & $<$\$1 a volumen actual \\
Re-entrenamiento peri\'odico & 1 corrida QLoRA equivalente a las ya ejecutadas (\$5--7), cadencia trimestral & $\sim$\$2 (amortizado) \\
...
\end{tabularx}
\\[0.3em]
{\scriptsize $^\ddagger$Inferencia, derivaci\'on: cada decisi\'on consume $\sim$1{,}000 \textit{tokens} ($\sim$900 de \textit{prompt} de indicadores $+$ $\sim$100 de respuesta JSON), de modo que 1{,}000 decisiones $\approx$ 1\,M \textit{tokens}. Local (Ollama) $\approx$ electricidad amortizada ($\sim$\$0.02/1k); API gestionada de un modelo de 7B a tarifa p\'ublica del orden de \$0.08/1M \textit{tokens} ($\sim$\$0.08/1k). Cifras de orden de magnitud; ambas rutas quedan muy por debajo de \$1/mes al volumen de decisiones actual. \textbf{Cadencia trimestral}: equilibra la frescura ante el \textit{drift} de r\'egimen ---que en cripto se mueve en escalas de semanas a meses--- contra el costo de validar cada reentrenamiento antes de promoverlo (\S\ref{sec:riesgos}).}
\end{table}
...
\item \textbf{Inferencia de bajo costo y baja latencia}: servido localmente con Ollama, el modelo cuesta $\sim$\$0.02 por cada 1{,}000 predicciones y responde en $\sim$300\,ms por decisi\'on --- una fracci\'on del costo de servir por API un modelo de la clase 70B ($\sim$\$0.08/1k); como alternativa sin operar GPU propia, Together AI lo sirve de forma gestionada a esa misma fracci\'on del costo. Conviene precisar que esta comparaci\'on de 70B es \'unicamente una referencia de costo de mercado: el \textit{baseline zero-shot} medido en este trabajo es el mismo modelo de 7B (\S3), no uno de 70B. El margen operativo mejora a medida que crece el volumen de decisiones.
\item \textbf{Retorno simulado positivo y consistente}: bajo la simulaci\'on hist\'orica m\'as conservadora (sin filtro de calidad), el modelo final promedia $+$0.42\,\% de retorno por operaci\'on sobre 11{,}294 operaciones en 81 d\'ias. Se reporta como \textbf{evidencia direccional, no como proyecci\'on de rentabilidad}: cada operaci\'on se trata como una apuesta unitaria sin \textit{compounding} ni l\'imite de capital concurrente, sin \textit{slippage} ni impacto de mercado, y el periodo de 81 d\'ias evaluado fue predominantemente alcista (\S\ref{sec:riesgos}). El acumulado bruto sin reinversi\'on de ese promedio ($+$4{,}768\,\%) ilustra la \emph{consistencia del signo}, no un retorno alcanzable en operaci\'on real; debe leerse como un \textbf{ranking comparativo entre modelos} bajo id\'entica metodolog\'ia, no como un retorno proyectado.
\end{itemize}
...
\item \textbf{Oportunidades de negocio adyacentes.} La misma infraestructura de ajuste de bajo costo ($<$\$20 por modelo) y la salida explicable habilitan extensiones naturales: ampliar el universo de activos cubiertos, exponer las se\~nales razonadas como un servicio a terceros (donde la justificaci\'on textual mejora la experiencia del usuario frente a una caja negra), o adaptar el mismo \textit{pipeline} a otros mercados (FX, acciones). Se mencionan como direcci\'on estrat\'egica, no como compromiso de este avance.
\end{itemize}

\subsection{S\'intesis costo--beneficio}

Combinando ambos lados en t\'erminos de negocio: el \textbf{costo total de propiedad del primer a\~no} es modesto y acotado ---menos de \$20 USD de I+D ya incurridos m\'as \$15--25 USD/mes de operaci\'on, esto es, aproximadamente \textbf{\$200--320 USD el primer a\~no} y \$180--300 USD/a\~no en adelante. Contra esa cifra, el beneficio \textbf{no debe expresarse todav\'ia como ingreso proyectado} ---la simulaci\'on hist\'orica no es una promesa de rentabilidad (\S\ref{sec:riesgos})--- sino como \textbf{riesgo evitado}: el componente que hoy opera en producci\'on (\textit{zero-shot}) tiene un \textit{profit factor} de 0.70, es decir, destruye capital de forma sostenida; el modelo ajustado lo lleva a 2.09 (rango profesional). El gasto recurrente de operaci\'on es, por tanto, trivial y conocido, mientras que el lado del beneficio es cualitativamente distinto: convertir un componente \emph{operativamente inviable} en uno \emph{viable}. La rentabilidad monetaria concreta queda deliberadamente sujeta a la validaci\'on en vivo de la Fase 2 antes de comprometer capital ---es exactamente la informaci\'on que el \textit{gate} de 4 semanas est\'a dise\~nado para producir.
...
\item \textbf{Memorizaci\'on por pre-entrenamiento}: descartada --- el per\'iodo de prueba (febrero--mayo 2026) es posterior al corte de conocimiento de Qwen~2.5 (septiembre 2024); el modelo no pudo haber memorizado la acci\'on de precio evaluada. \textit{Verificado.}
\end{itemize}
\end{riskpanel}
\end{minipage}\hfill
\begin{minipage}{0.485\linewidth}
...
\item \textbf{Sin freno autom\'atico (\textit{circuit breaker})}: no existe a\'un un l\'imite de p\'erdida agregada que detenga al agente aut\'onomo si empieza a perder r\'apido en operaci\'on real, m\'as all\'a del \textit{stop-loss} por operaci\'on individual. Es un control obligatorio antes de comprometer capital real.
\item \textbf{?`Sobreajuste o conjunto de prueba insuficiente?} Descartado --- el conjunto de prueba se eval\'ua completo (11{,}294 muestras, sin submuestreo) y la p\'erdida de validaci\'on no diverge durante el entrenamiento. \textit{Verificado.}
\end{itemize}
\end{riskpanel}
\end{minipage}\hfill
\begin{minipage}{0.485\linewidth}
...
\item \textbf{Dependencia de un \'unico \textit{exchange}}: todo el sistema depende del acceso continuo a la API de Binance bajo sus t\'erminos actuales; una restricci\'on regulatoria, un \textit{rate-limit} o un bloqueo por jurisdicci\'on ---frecuentes en cripto--- interrumpir\'ia la operaci\'on. No hay hoy un \textit{exchange} de respaldo integrado.
\item \textbf{Sin proceso formal de gobernanza de riesgo de modelo} (documentaci\'on, validaci\'on y monitoreo continuo); este documento y la auditor\'ia interna son un punto de partida, no un proceso institucionalizado. En particular, falta una prueba de regresi\'on entre versiones reentrenadas: cada reentrenamiento puede introducir puntos ciegos distintos sin un control que lo detecte (Babic et al., 2021).
\end{itemize}
\end{riskpanel}
\end{minipage}
...
{\footnotesize Cada secci\'on integra un entregable previo del proyecto: \S1 (Avance 0 --- Definici\'on del proyecto), \S2 (Avance 1 --- An\'alisis Exploratorio de Datos), \S3 (Avances 5--6 --- comparaci\'on de familias de modelos y selecci\'on de la variante final de producci\'on), \S4 (Avance 6 --- Recomendaciones de implementaci\'on). El an\'alisis de costo-beneficio (\S5) y de riesgos (\S6) se elabor\'o para este Avance 7, con base en la auditor\'ia adversarial documentada en \texttt{docs/qlora/AUDIT\_QLORA\_88PCT.md} y en los resultados finales del reporte t\'ecnico \texttt{Conclusions\_Avance6.tex}. C\'odigo y datos: \url{https://github.com/alejandrogonzalz/trading_management}.}

\subsection*{\sffamily Referencias}
{\footnotesize
\begin{itemize}[leftmargin=1.4em]
\item B.~Babic, I.~G.~Cohen, T.~Evgeniou y S.~Gerke (2021). ``When Machine Learning Goes Off the Rails.'' \textit{Harvard Business Review}, ene.--feb.~2021. (Inestabilidad de modelos de ML entre reentrenamientos y necesidad de gobernanza/regresi\'on entre versiones, \S6.)
